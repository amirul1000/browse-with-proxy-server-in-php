...
<h5 class="font-20 mt-15 mb-1"><?php echo str_replace('_',' ','Contact Form'); ?></h5>
<?php
echo $this->session->flashdata('msg');
?>
<!--Form to save data-->
<?php echo form_open_multipart('contact/save/'.$contact['id'],array("class"=>"form-horizontal")); ?>
<div class="card">
			<div class="card-body">
				...
				<div class="form-group">
					...
					<div class="col-md-8">
						<textarea name="comment" id="comment" class="form-control"
							rows="4" /><?php echo ($this->input->post('comment') ? $this->input->post('comment') : $contact['comment']); ?></textarea>
					</div>
				</div>
		...
		<div class="form-group">
			<div class="col-sm-offset-4 col-sm-8">
				<button type="submit" class="btn btn-success"><?php if(empty($contact['id'])){?>Submit<?php }else{?>Update<?php } ?></button>
			</div>
		</div>
<?php echo form_close(); ?>
