...
class Subscriber extends CI_Controller
{
    ...
    {
        $limit = 10;
        $data['subscriber'] = $this->Subscriber_model->get_limit_subscriber($limit, $start);
        // pagination
        $config['base_url'] = site_url('admin/subscriber/index');
        $config['total_rows'] = $this->Subscriber_model->get_count_subscriber();
        $config['per_page'] = 10;
        ...
        $data['_view'] = 'admin/subscriber/index';
    ...
    function save($id = - 1)
    {
        ...
        if (isset($id) && $id > 0) {
            ...
            if (isset($_POST) && count($_POST) > 0) {
                ...
                redirect('admin/subscriber/index');
            } else {
                $data['_view'] = 'admin/subscriber/form';
                $this->load->view('layouts/admin/body', $data);
            }
        } // save
        else {
    ...
    function details($id)
    {
        $data['subscriber'] = $this->Subscriber_model->get_subscriber($id);
        $data['id'] = $id;
        $data['_view'] = 'admin/subscriber/details';
        $this->load->view('layouts/admin/body', $data);
    }
    ...
    function search($start = 0)
    {
        ...
        // pagination
        $config['base_url'] = site_url('admin/subscriber/search');
        $this->db->reset_query();
        $this->db->like('id', $key, 'both');
        $this->db->or_like('subscription_item_id', $key, 'both');
        $this->db->or_like('subscriber_users_id', $key, 'both');
    ...
    function export($export_type = 'CSV')
    {
        if ($export_type == 'CSV') {
            ...
            // get data
            $this->db->order_by('id', 'desc');
            $subscriberData = $this->Subscriber_model->get_all_subscriber();
            // file creation
            ...
            // get the HTML
            ob_start();
            include (APPPATH . 'views/admin/subscriber/print_template.php');
            $html = ob_get_clean();
